...
trait scores {
  ...
  def writeList(list: List[(String,String)], file: File) = {
    val writer = CSVWriter.open(file.toJava)
    list.foreach(l=>writer.writeRow(List(l._1,l._2)))
    writer.close()
  }
  def countTruePositives(elements: List[(String,String)], prediction: List[(String,String)]) = {
    prediction.count(elements.contains)
  }
  def truePositives(elements: List[(String,String)], prediction: List[(String,String)]) = {
    prediction.filter(elements.contains)
  }
...
//    println("f1Score " + f1Score)
  ...
  def removeDoubles(elements: List[(String,String)]) = elements.foldLeft(List[(String,String)]())((l:List[(String,String)], matching:(String,String))=>if (l.contains(matching)) {println(matching);l} else l:+matching)
}
