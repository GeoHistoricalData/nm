...
/**
  */
trait scores {
  implicit def File2List(file: File) = CSVReader.open(file.toJava).all().map(l=>(l(0),l(1)))
  implicit def JavaFile2List(file: java.io.File) = CSVReader.open(file).all().map(l=>(l(0),l(1)))
  ...
  def fscore(elements: List[(String,String)])(prediction: List[(String,String)]) = {
    compute(elements)(prediction)._3
  }
  def fscore(elementsFile: java.io.File)(prediction: List[(String,String)]): Double = {
    val r = compute(JavaFile2List(elementsFile))(prediction)
    println(r)
    r._3
  }
}
